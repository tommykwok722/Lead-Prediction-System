...
# 1.Plotting the Histogram to visualise the distribution of Avg_Account_Balance
data.imputed %>%
...
# 2. Which communication channel is used the most, and how well is it working?
...
ggplot(data_percent, aes(x = Channel_Code, y = n, fill = as.factor(Target))) +
  ...
  labs(x = "Channel Code", y = "No. of Customers", fill = "Target",title = "Target Distribution across Channel Codes")

# 3. Sort the average account balance in descending order to check if there are any outliers
data.imputed %>%
...
### 1.1 Logistic Regression: all attributes on training.undersampled
```{r}
model_LR <- glm(Target~., training.undersampled, family = "binomial")
...
### 1.2 Logistic Regression: 3 attributes on training.undersampled
```{r}
model_LR1 <- glm(Target~Registration+Age+Channel_Code, training.undersampled, family = "binomial")
...
### 1.3 Logistic Regression: 5 attributes on training.undersampled
```{r}
model_LR2 <- glm(Target~Registration+Age+Channel_Code+Vintage+Credit_Product , training.undersampled, family = "binomial")
...
### 1.4 Logistic Regression: 7 attributes on training.undersampled
```{r}
model_LR3 <- glm(Target~Registration+Age+Channel_Code+Vintage+Credit_Product+Region_Code+Occupation_Salaried , training.undersampled, family = "binomial")
...
### 2.1 Decision Tree: All attributes on training.undersampled
```{r}
model_DT <- C5.0(Target~., data = training.undersampled)
...
### 2.2 Decision Tree: 3 attributes on training.undersampled
```{r}
model_DT1 <- C5.0(Target~Registration+Age+Channel_Code, data = training.undersampled)
...
### 2.3 Decision Tree: 5 attributes on training.undersampled
```{r}
model_DT2 <- C5.0(Target~Registration+Age+Channel_Code+Vintage+Credit_Product, data = training.undersampled)
...
### 2.4 Decision Tree: 7 attributes on training.undersampled
```{r}
model_DT3 <- C5.0(Target~Registration+Age+Channel_Code+Vintage+Credit_Product+Region_Code+Occupation_Salaried, data = training.undersampled)
...
### 3.1 Linear Discriminant Analysis: All attributes on training.undersampled
```{r}
# Build a Linear Discriminant Analysis model (classification method)
model_LDA <- lda(Target~., data = training.undersampled)
...
### 3.2 Linear Discriminant Analysis: 3 attributes on training.undersampled
```{r}
model_LDA1 <- lda(Target~Registration+Age+Channel_Code, data = training.undersampled)
...
### 3.3 Linear Discriminant Analysis: 5 attributes on training.undersampled
```{r}
model_LDA2 <- lda(Target~Registration+Age+Channel_Code+Vintage+Credit_Product, data = training.undersampled)
...
### 3.4 Linear Discriminant Analysis: 7 attributes on training.undersampled
```{r}
model_LDA3 <- lda(Target~Registration+Age+Channel_Code+Vintage+Credit_Product+Region_Code+Occupation_Salaried, data = training.undersampled)
...
### 4.1 Support Vector Machine: All attributes on training.undersampled
```{r}
# Build the SVM model using training.undersampled
model_SVM <- svm(Target~., data= training.undersampled, kernel = 'radial', scale = T, probability = T)
...
### 4.2 Support Vector Machine: 3 attributes on training.undersampled
```{r}
model_SVM1 <- svm(Target~Registration+Age+Channel_Code, data= training.undersampled, kernel = 'radial', scale = T, probability = T)
...
### 4.3 Support Vector Machine: 5 attributes on training.undersampled
```{r}
model_SVM2 <- svm(Target~Registration+Age+Channel_Code+Vintage+Credit_Product, data= training.undersampled, kernel = 'radial', scale = T, probability = T)
...
### 4.4 Support Vector Machine: 7 attributes on training.undersampled
```{r}
model_SVM3 <- svm(Target~Registration+Age+Channel_Code+Vintage+Credit_Product+Region_Code+Occupation_Salaried, data= training.undersampled, kernel = 'radial', scale = T, probability = T)
...
### 5.1 Random Forest: All attributes on training.undersampled
```{r}
set.seed(123)

# Build Random Forest model and assign it to model_RF
model_RF <- randomForest(Target~., training.undersampled)
...
### 5.2 Random Forest: 3 attributes on training.undersampled
```{r}
set.seed(123)
model_RF1 <- randomForest(Target~Registration+Age+Channel_Code, training.undersampled)
...
### 5.3 Random Forest: 5 attributes on training.undersampled
```{r}
set.seed(123)
model_RF2 <- randomForest(Target~Registration+Age+Channel_Code+Vintage+Credit_Product, training.undersampled)
...
### 5.4 Random Forest: 7 attributes on training.undersampled
```{r}
set.seed(123)
model_RF3 <- randomForest(Target~Registration+Age+Channel_Code+Vintage+Credit_Product+Region_Code+Occupation_Salaried, training.undersampled)
...
# Testing Accuracy
test.accuracy_DT <- confusionMatrix(pred_DT, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_DT1 <- confusionMatrix(pred_DT1, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_DT2 <- confusionMatrix(pred_DT2, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_DT3 <- confusionMatrix(pred_DT3, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

test.accuracy_LDA <- confusionMatrix(pred_LDA, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_LDA1 <- confusionMatrix(pred_LDA1, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_LDA2 <- confusionMatrix(pred_LDA2, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_LDA3 <- confusionMatrix(pred_LDA3, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

test.accuracy_LR <- confusionMatrix(model_LR_class, test$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
test.accuracy_LR1 <- confusionMatrix(model_LR_NoSmot_class1, test$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
test.accuracy_LR2 <- confusionMatrix(model_LR_NoSmot_class2, test$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
test.accuracy_LR3 <- confusionMatrix(model_LR_NoSmot_class3, test$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']

test.accuracy_RF <- confusionMatrix(prediction_RF, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_RF1 <- confusionMatrix(prediction_RF1, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_RF2 <- confusionMatrix(prediction_RF2, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_RF3 <- confusionMatrix(prediction_RF3, test$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

test.accuracy_SVM <- confusionMatrix(prediction_SVM, test$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_SVM1 <- confusionMatrix(prediction_SVM1, test$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_SVM2 <- confusionMatrix(prediction_SVM2, test$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
test.accuracy_SVM3 <- confusionMatrix(prediction_SVM3, test$Target, positive='1', mode = "prec_recall")$overall['Accuracy']

test.accuracy <- data.frame(c(test.accuracy_DT1, test.accuracy_DT2, test.accuracy_DT3, test.accuracy_DT),
           c(test.accuracy_LDA1, test.accuracy_LDA2, test.accuracy_LDA3, test.accuracy_LDA),
           c(test.accuracy_LR1, test.accuracy_LR2, test.accuracy_LR3, test.accuracy_LR),
           c(test.accuracy_RF1, test.accuracy_RF2, test.accuracy_RF3, test.accuracy_RF),
           c(test.accuracy_SVM1, test.accuracy_SVM2, test.accuracy_SVM3, test.accuracy_SVM))

colnames(test.accuracy) <- c("DT", "LDA", "LR", "RF", "SVM")
rownames(test.accuracy) <- c("3 Variables", "5 Variables", "7 Variables", "All Variables")

# Training Prediction
pred.train_DT <- predict(model_DT, training, type = "class")
pred.train_DT1 <- predict(model_DT1, training, type = "class")
pred.train_DT2 <- predict(model_DT2, training, type = "class")
pred.train_DT3 <- predict(model_DT3, training, type = "class")

pred.train_LDA <- predict(model_LDA, training)$class
pred.train_LDA1 <- predict(model_LDA1, training)$class
pred.train_LDA2 <- predict(model_LDA2, training)$class
pred.train_LDA3 <- predict(model_LDA3, training)$class

pred.train_LR <- predict(model_LR, training, type = "response")
pred.train_LR <- as.factor(ifelse(pred.train_LR > 0.5, 1, 0))
pred.train_LR1 <- predict(model_LR1_NoSmot, training, type = "response")
pred.train_LR1 <- as.factor(ifelse(pred.train_LR1 > 0.5, 1, 0))
pred.train_LR2 <- predict(model_LR2_NoSmot, training, type = "response")
pred.train_LR2 <- as.factor(ifelse(pred.train_LR2 > 0.5, 1, 0))
pred.train_LR3 <- predict(model_LR3_NoSmot, training, type = "response")
pred.train_LR3 <- as.factor(ifelse(pred.train_LR3 > 0.5, 1, 0))

pred.train_RF <- predict(model_RF, training, type = "class")
pred.train_RF1 <- predict(model_RF1, training, type = "class")
pred.train_RF2 <- predict(model_RF2, training, type = "class")
pred.train_RF3 <- predict(model_RF3, training, type = "class")

pred.train_SVM <- predict(model_SVM, training, type = "class")
pred.train_SVM1 <- predict(model_SVM1, training, type = "class")
pred.train_SVM2 <- predict(model_SVM2, training, type = "class")
pred.train_SVM3 <- predict(model_SVM3, training, type = "class")

# Training Accuracy
train.accuracy_DT <- confusionMatrix(pred.train_DT, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_DT1 <- confusionMatrix(pred.train_DT1, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_DT2 <- confusionMatrix(pred.train_DT2, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_DT3 <- confusionMatrix(pred.train_DT3, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

train.accuracy_LDA <- confusionMatrix(pred.train_LDA, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_LDA1 <- confusionMatrix(pred.train_LDA1, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_LDA2 <- confusionMatrix(pred.train_LDA2, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_LDA3 <- confusionMatrix(pred.train_LDA3, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

train.accuracy_LR <- confusionMatrix(pred.train_LR, training$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
train.accuracy_LR1 <- confusionMatrix(pred.train_LR1, training$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
train.accuracy_LR2 <- confusionMatrix(pred.train_LR2, training$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']
train.accuracy_LR3 <- confusionMatrix(pred.train_LR3, training$Target, positive = "1", mode = "prec_recall")$overall['Accuracy']

train.accuracy_RF <- confusionMatrix(pred.train_RF, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_RF1 <- confusionMatrix(pred.train_RF1, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_RF2 <- confusionMatrix(pred.train_RF2, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_RF3 <- confusionMatrix(pred.train_RF3, training$Target, positive ='1', mode = "prec_recall")$overall['Accuracy']

train.accuracy_SVM <- confusionMatrix(pred.train_SVM, training$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_SVM1 <- confusionMatrix(pred.train_SVM1, training$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_SVM2 <- confusionMatrix(pred.train_SVM2, training$Target, positive='1', mode = "prec_recall")$overall['Accuracy']
train.accuracy_SVM3 <- confusionMatrix(pred.train_SVM3, training$Target, positive='1', mode = "prec_recall")$overall['Accuracy']

train.accuracy <- data.frame(c(train.accuracy_DT1, train.accuracy_DT2, train.accuracy_DT3, train.accuracy_DT),
           c(train.accuracy_LDA1, train.accuracy_LDA2, train.accuracy_LDA3, train.accuracy_LDA),
           c(train.accuracy_LR1, train.accuracy_LR2, train.accuracy_LR3, train.accuracy_LR),
           c(train.accuracy_RF1, train.accuracy_RF2, train.accuracy_RF3, train.accuracy_RF),
           c(train.accuracy_SVM1, train.accuracy_SVM2, train.accuracy_SVM3, train.accuracy_SVM))

colnames(train.accuracy) <- c("DT", "LDA", "LR", "RF", "SVM")
rownames(train.accuracy) <- c("3 Variables", "5 Variables", "7 Variables", "All Variables")

# Check if test.accuracy > train.accuracy
check.accuracy <- test.accuracy - train.accuracy
```
...
# Computational Time (Without Parallel Computing)
comp.time_DT <- system.time(predict(model_DT, test, type = "class"))["elapsed"]
comp.time_LDA <- system.time(predict(model_LDA, test, type = "class"))["elapsed"]
comp.time_LR <- system.time(predict(model_LR, test, type = "response"))["elapsed"]
comp.time_RF <- system.time(predict(model_RF, test, type = "class"))["elapsed"]
comp.time_SVM <- system.time(predict(model_SVM, test, type = "class"))["elapsed"]

comp.time <- data.frame(comp.time_DT, comp.time_LDA, comp.time_LR, comp.time_RF, comp.time_SVM)
colnames(comp.time) <- c("DT", "LDA", "LR", "RF", "SVM")

# Detect the number of available cores
no_cores <- detectCores()

# Split test data into chunks
test_chunks <- split(test, rep(1:4, length.out = nrow(test)))

# Computational Time (With Parallel Computing)
comp.time.parallel_DT <- system.time(mclapply(test_chunks, function(chunk) predict(model_DT, newdata = chunk), mc.cores = no_cores))["elapsed"]
comp.time.parallel_LDA <- system.time(mclapply(test_chunks, function(chunk) predict(model_LDA, newdata = chunk), mc.cores = no_cores))["elapsed"]
comp.time.parallel_LR <- system.time(mclapply(test_chunks, function(chunk) predict(model_LR, newdata = chunk), mc.cores = no_cores))["elapsed"]
comp.time.parallel_RF <- system.time(mclapply(test_chunks, function(chunk) predict(model_RF, newdata = chunk), mc.cores = no_cores))["elapsed"]
comp.time.parallel_SVM <- system.time(mclapply(test_chunks, function(chunk) predict(model_SVM, newdata = chunk), mc.cores = no_cores))["elapsed"]

comp.time.parallel <- data.frame(comp.time.parallel_DT, comp.time.parallel_LDA, comp.time.parallel_LR,
                                 comp.time.parallel_RF, comp.time.parallel_SVM)
colnames(comp.time.parallel) <- c("DT", "LDA", "LR", "RF", "SVM")

# Time Comparison
time_compare <- rbind(comp.time, comp.time.parallel, (comp.time.parallel-comp.time)/comp.time)
row.names(time_compare) = c("Without Parallel", "With Parallel", "% Change")
```
